...
const FindJobHeader: FunctionComponent<FrameComponentProps> = ({ activeTab, setActiveTab, userId, projects, setProjects, setProjectsWork, projectsWork }) => {
  const [skills, setSkills] = useState<UserSkillsRelationAttributes[]>([]);
  const [projectServices, setProjectServices] = useState<ProjectsServicesListAttributes[]>([]);
  const [allSkills, setAllSkills] = useState<SkillsAttributes[]>([]);
  const [allServices, setAllServices] = useState<ServicesAttributes[]>([]);

  const [filteredServices, setFilteredServices] = useState<ServicesAttributes[]>([]);
  ...
  const fetchSkillsByUser = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/userSkills/${userId}`);
      if (!response.ok) {
  ...
  const fetchAllSkills = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/competence/all`);
      if (!response.ok) {
  ...
  const fetchAllServices = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/service/all`);
      if (!response.ok) {
  ...
  const fetchProjectServicesList = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/projectsServicessList/all`);
      if (!response.ok) {
  ...
  useEffect(() => {
    if (activeTab === 1) {
      const filteredDomaines = allSkills
        .filter((skill) => skills.some((userSkill) => userSkill.skills_id === skill.id))
  ...
  useEffect(() => {
    if (activeTab === 1) {
      const filteredServices = allServices.filter(service => domaines.includes(service.domaine));
  ...
  const onSavedJobsTextClick = useCallback(async () => {
    ...
    try {
      ...
      const savedProjectsResponse = await fetch(`http://localhost:3001/api/userProjectRelation/${userId}`);
      ...
      const allProjectsResponse = await fetch('http://localhost:3001/api/project/all');
      ...
      const projectsWork = allProjects.filter((project: ProjectData) => savedProjects.some((savedProject:UserProjectsRelationAttributes) => savedProject.projectId === project.id));
      ...
      <div className="self-stretch flex flex-row items-start justify-between max-w-full gap-[20px] mq1050:flex-wrap mq1050:justify-center">
        <div className="w-[649.3px] flex flex-col items-start justify-start gap-[38.5px] max-w-full mq750:gap-[19px_38.5px]">
          ...
          <div className="w-[633.5px] flex flex-row items-start justify-between gap-[20px] max-w-full text-justify text-5xl-2 mq750:flex-wrap">
            <div className={`h-[44.6px] relative tracking-[-0.01em] leading-[145.45%] font-light flex items-center cursor-pointer mq450:text-lgi mq450:leading-[28px] hover:underline ${activeTab === 1 ? "underline" : ""
              }`}
              onClick={onBestMatchesClick}>
              Best Matches
            </div>
            <div
              className={`h-[44.6px] relative tracking-[-0.01em] leading-[145.45%] font-light flex items-center cursor-pointer mq450:text-lgi mq450:leading-[28px] hover:underline ${activeTab === 2 ? "underline" : ""
                }`}
            ...
            <div
              className={`h-[44.6px] relative tracking-[-0.01em] leading-[145.45%] font-light flex items-center cursor-pointer mq450:text-lgi mq450:leading-[28px] hover:underline ${activeTab === 3 ? "underline" : ""
                }`}
        ...
        <div className="relative flex flex-col items-start justify-start gap-2">
          <TextField
              ...
              if (searchTerm != "") {
                if (e.key === 'Enter') {
                  ...
                  if (searchResultsElement) {
                    window.scroll({
                      top: searchResultsElement.getBoundingClientRect().top + window.pageYOffset,
                      behavior: 'smooth'
                    });
                  }
                }
            ...
            InputProps={{
              endAdornment: (
                <IconButton onClick={searchTerm !== "" ? searchProjects : undefined} size="small">
                  <SearchIcon />
                </IconButton>
